...
import { eliminarJugadorDeSala } from '@/core/db/queries/delete'
import { agregarJugadorASala, crearJugador, crearSala } from '@/core/db/queries/insert'
import { getAllSalasPorJugadorId, getJugadorPorId } from '@/core/db/queries/select'
...
export async function actionCrearSala (initialState: unknown, formData: FormData) {
  try {
    ...
    const { data: creador } = await getUser()
    ...
    const salasPrevias = await getAllSalasPorJugadorId(creadorId)
    ...
    if (!jugador) {
      jugador = await crearJugador(creador)
    }
...
export async function actionIngresarSala (initialState: unknown, formData: FormData) {
  ...
  try {
    const data = await agregarJugadorASala(salaId, jugadorId)
...
export async function actionAbandonarSala (initialState: unknown, formData: FormData) {
  const salaId = formData.get('sala_id') as string
  const jugadorId = formData.get('jugador_id') as string
  ...
  try {
    ...
    if (!data) throw new Error('No se pudo abandonar la sala')
